...
def run_sweep(period, out_filename):
    print(f"\n--- Running Sweep for BTC-USD ({period}) ---")
    df = fetch_data("BTC-USD", period)
    if df is None:
    ...
    print(f"Testing {len(combinations)} parameter suites...")
    results = []
    ...
    res_df = pd.DataFrame(results).sort_values(by='Total_Return_%', ascending=False)
    res_df.to_csv(out_filename, index=False)

    print(f"Top 5 BTC Suites ({period}):")
