...
def main():
    print("Fetching real BTC-USD hourly data using yfinance (past 180 days / 6 months)...")
    df = yf.download("BTC-USD", interval="1h", period="180d", progress=False)
    ...
    print(f"Running {len(combinations)} combinations against 6-month real data...")
    ...
    results_df.to_csv('real_data_backtest_results_6mo.csv', index=False)

    buy_and_hold = (df['Close'].iloc[-1] - df['Close'].iloc[0]) / df['Close'].iloc[0] * 100

    print("\n=== Top 10 Real Data Combinations (6 Months) ===")
    print(results_df.head(10).to_string())
    print(f"\nBaseline Buy & Hold Return: {buy_and_hold:.2f}%")
    print("Results saved to real_data_backtest_results_6mo.csv")
